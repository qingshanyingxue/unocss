...
- [快速开始](#快速开始)
- [项目结构](#项目结构)
- [UnoCSS 配置详解](#unocss-配置详解)
- [功能模块学习指南](#功能模块学习指南)
  - [1. 基础入门](#1-基础入门)
  - [2. 属性化模式 Attributify](#2-属性化模式-attributify)
  - [3. 变体组 Variant Group](#3-变体组-variant-group)
  - [4. @apply 指令 Directives](#4-apply-指令-directives)
  - [5. 快捷方式 Shortcuts](#5-快捷方式-shortcuts)
  - [6. 图标系统 Icons](#6-图标系统-icons)
  - [7. 排版预设 Typography](#7-排版预设-typography)
  - [8. 响应式设计](#8-响应式设计)
  - [9. 动画与过渡](#9-动画与过渡)
  - [10. 自定义规则 Rules](#10-自定义规则-rules)
  - [11. Flex 布局速查](#11-flex-布局速查)
  - [12. 常用 Class 速查表](#12-常用-class-速查表)
- [常用 Class 速记](#常用-class-速记)
- [常见问题 FAQ](#常见问题-faq)
...
本项目是一个 **交互式 UnoCSS 教学应用**，通过 12 个可切换的 Tab 页面，以实际渲染效果 + 代码对照的方式，系统学习 UnoCSS 的各项功能。

每个模块都包含：
- 概念说明（是什么）
- 代码对比（传统写法 vs UnoCSS 写法）
- 实时交互演示（动手看效果）
- 学习要点总结（背口诀）
...
| 包名 | 类型 | 作用 |
|------|------|------|
| `unocss` | 核心 | 主引擎，Vite 插件入口 |
| `@unocss/preset-wind3` | 预设 | Tailwind/Wind 兼容，最常用 |
| `@unocss/preset-attributify` | 预设 | 属性化写法支持 |
| `@unocss/preset-icons` | 预设 | 纯 CSS 图标（10万+） |
| `@unocss/preset-typography` | 预设 | prose 文章排版 |
| `@unocss/transformer-directives` | 转换器 | @apply / @screen 指令 |
| `@unocss/transformer-variant-group` | 转换器 | 变体组语法糖 |
| `@unocss/transformer-attributify-jsx` | 转换器 | JSX 中支持属性化 |
...
export default defineConfig({
  // ===== 预设 Presets =====
  presets: [
    presetWind3(),         // 兼容 Tailwind CSS 语法，最常用
    presetAttributify(),   // 支持属性化写法
    presetIcons({          // 纯 CSS 图标，支持 10 万+图标
      scale: 1.2,          // 图标默认放大 1.2 倍
      cdn: 'https://esm.sh/', // 图标按需从 CDN 加载
    }),
    presetTypography(),    // prose 文章排版预设
  ],
...
export default defineConfig({
  plugins: [
    UnoCSS(),  // ⚠️ 必须放在 vue() 之前
    vue(),
  ],
})
```

> **注意**：`UnoCSS()` 插件必须注册在 `vue()` 之前，否则部分功能（如变体组转换）可能不生效。
...
**核心概念**：UnoCSS 是即时按需的原子化 CSS 引擎，每个 class 在运行时生成对应 CSS，零解析开销。
...
> 口诀：**数字 × 4 = 像素值**，如 `p-6` = 24px，`m-4` = 16px
...
把同一变体（hover/focus/md/dark）的多个工具类合并书写：
...
<!-- ✅ 变体组写法：hover: 只写1次 -->
hover:(bg-red-500 text-white scale-105 shadow-lg)
```
...
| 变体 | 普通写法 | 变体组写法 |
|------|----------|------------|
| hover | `hover:bg-red hover:text-white` | `hover:(bg-red text-white)` |
| focus | `focus:ring-2 focus:border-blue` | `focus:(ring-2 border-blue)` |
| md: | `md:text-lg md:p-8` | `md:(text-lg p-8)` |
| dark: | `dark:bg-gray-900 dark:text-white` | `dark:(bg-gray-900 text-white)` |
| 嵌套 | `md:hover:text-xl md:hover:bg-pink` | `md:hover:(text-xl bg-pink)` |

---

### 4. @apply 指令 Directives
...
// uno.config.ts
shortcuts: {
  'btn': 'px-4 py-2 rounded-md text-white font-semibold cursor-pointer',
...
**Shortcut vs @apply 对比**：

| 特性 | Shortcut | @apply |
|------|----------|--------|
| 定义位置 | uno.config.ts | 组件 style 标签 |
| 作用范围 | 全局 | 当前组件 |
| 支持继承 | ✅ btn-primary 包含 btn | ❌ |
| 推荐场景 | 设计系统基础组件 | 组件级自定义样式 |
...
<div class="i-material-symbols-home text-4xl" />
```
...
| 集合名 | 图标数量 | 风格 |
|--------|----------|------|
| `carbon` | 2000+ | IBM 风格，简洁 |
| `mdi` | 7000+ | Material Design |
| `heroicons` | 300+ | Tailwind 官方 |
| `tabler` | 5000+ | 线性风格 |
| `material-symbols` | 3000+ | Google Material |

**图标浏览**：https://icon-sets.iconify.design

**控制方式**：
- 颜色：`text-red-500`（图标是 currentColor）
- 大小：`text-2xl`（本质是 font-size）
- 状态：`hover:text-blue-500`
...
| 场景 | 写法 |
|------|------|
| 导航栏（Logo 左 + 菜单右） | `flex items-center justify-between px-4` |
...
| 左固定右自适应 | `flex` + 左 `w-48 flex-shrink-0` + 右 `flex-1` |
| 等分三列 | `flex` + 三个子元素各 `flex-1` |

**Flex 常见坑**：
- `flex-1` 不生效 → 父元素必须有 `flex`
- 文字溢出 → 加 `min-w-0` 或 `overflow-hidden truncate`
- `items-center` 没效果 → items 控制的是交叉轴，不是主轴
- `gap` vs `space-x` → 优先用 `gap-4`，更简洁且无首元素 margin 问题
...
| 前缀 | 含义 | 示例 |
|------|------|------|
| `p` | padding 内边距 | `p-4` = 16px |
| `m` | margin 外边距 | `m-4` = 16px |
| `w` | width 宽度 | `w-full` = 100% |
| `h` | height 高度 | `h-screen` = 100vh |
| `text` | 字体/文字 | `text-xl` = 1.25rem |
| `bg` | background 背景 | `bg-blue-500` |
...
| `focus:` | 输入框聚焦 | `focus:ring-2` |
| `active:` | 鼠标按下 | `active:scale-95` |
| `disabled:` | 元素禁用 | `disabled:opacity-50` |
| `group-hover:` | 父元素悬停 | `group-hover:text-white` |
...
UnoCSS 完全兼容 Tailwind 语法（通过 `presetWind3`），同时额外提供：
- 属性化写法（Attributify）
- 变体组语法糖（Variant Group）
- 纯 CSS 图标（Icons）
- 自定义规则（Rules）
- 更快的编译速度
...
2. **动态类名问题**：UnoCSS 默认扫描字符串，动态拼接如 `` `bg-${color}-500` `` 需要确保 `color` 值在扫描范围内
3. **忘记开启预设**：如属性化写法需要 `presetAttributify()`

### Q：如何查看 UnoCSS 生成了哪些 CSS？

打开浏览器 DevTools → Elements 面板，选中元素后在右侧 Styles 面板可以看到 UnoCSS 即时生成的 CSS 规则。

### Q：变体组写法在哪些地方有效？

需要开启 `transformerVariantGroup()`，在 HTML class 属性、Vue SFC 的 class 绑定中均有效：
...
### Q：Shortcuts 和 @apply 该用哪个？

| 场景 | 推荐 |
|------|------|
| 全局复用的组件样式（btn/card） | Shortcuts |
| 组件内部私有样式 | @apply |
| 需要继承（btn-primary 包含 btn） | Shortcuts |
| 配合 CSS 变量/伪元素 | @apply |
